...
| `SUBTITLES_RATE_LIMIT_HOLD_MS` | `600000` | After a platform answers 429 to a subtitle download, the server stops asking that platform for subtitles for this long and answers `rate_limited` right away. Each repeat doubles the wait, up to six times this value; a successful download clears it. Metadata is not held back |
