...
- **One yt-dlp run answers video info, the track list and chapters:** whichever of the three is asked for first runs yt-dlp once, fills all three cache entries and hands the JSON to the transcript path for the track address. Calls for the same video that arrive together share that one run instead of each starting their own. The canary still runs its own probe and writes nothing to the cache.
