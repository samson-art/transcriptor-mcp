...
- The `rate_limited` message no longer says "wait a few minutes, then retry once". These limits last hours, and the sentence invited exactly the repeats that were measured (two addresses asked 10 and 12 times in a quarter of an hour). It now says most requests to the platform keep failing while the limit lasts, that it can last hours, and not to retry this request. When the server is holding the platform back it adds the facts it has: how long the platform has been refusing, and how long the server will leave it alone. It states no second next step, and it carries no wall-clock time, which would be ambiguous for a limit that outlives the day.
