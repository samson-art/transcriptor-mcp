...
- **`get_playlist_transcripts` needs `lang`.** Without it the tool used to fetch English and now asks for `lang` before it runs anything. One run cannot pick each video's original language, and a pattern for YouTube's original-language tracks would request every audio track of a dubbed video.
