...
- **`.env.example` lists five env vars the server already read:** `YT_DLP_FRAME_TIMEOUT`, `YT_DLP_JS_RUNTIMES`, `YT_DLP_REMOTE_COMPONENTS`, `YT_DLP_NO_WARNINGS` and `YT_DLP_IGNORE_NO_FORMATS`, with their defaults. Nothing about them changed. Do not set `YT_DLP_NO_WARNINGS=1` while `YT_DLP_IGNORE_NO_FORMATS` is on: the server then cannot tell a private, removed or bot-checked video from a normal one. A test now fails when the code reads an env var that the file does not name.
