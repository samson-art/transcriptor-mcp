...
- **During a caption 429 hold, a track from before the hold ended a canary failure streak.** The canary set `transcriptor_canary_ok` to 1 while the platform refused every caption request. The canary now does not stand down during a hold. Its probe stops at the hold, with no request to the platform.
