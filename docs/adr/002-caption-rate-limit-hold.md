# 002. After a caption 429, hold that platform's caption path in the process; only a delivered track resets the strikes
...
YouTube's caption limit is keyed to the server's outbound address. Two limits lasted about a day each (CHANGELOG 1.5.2). A third was lifted at once by changing the address (PR #39), which shows what it is keyed to. While a limit lasted, every failed call spent more caption requests and the canary kept probing, which fed the limit. Callers told to "wait a few minutes, then retry once" repeated the call 10–12 times.

From 1.5.2 to 1.5.7 a refusal counted as a repeat strike only if it came within the base wait of the previous wait ending. With sparse traffic every 429 read as strike 1, so the wait never grew and nothing showed a ban.
...
- Calls in flight that report the same limit count once. Any refusal after a wait has run out is the next strike, however late it comes. The wait doubles 10 → 20 → 40 minutes, capped at 60 (at the default base).
...
- **Count a repeat only within a time window** (1.5.2–1.5.7). Cannot see a ban when traffic is sparse.
...
- **Clear the hold on any successful run.** Rejected: a run with no track may never have asked the caption endpoint.
...
- Uncached transcripts on a held platform fail for up to an hour, even if the limit lifts sooner.

## Don't

- Don't add a time decay to the strike count. It looks like a bug, but it brings back the pre-1.5.8 blindness.
- Don't clear the hold on any successful exit, count parallel 429s as separate strikes, move the state to Redis, extend the hold to metadata, or soften the text to "try again in a few minutes".
